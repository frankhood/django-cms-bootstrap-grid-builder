<template>
  <div class="myComponent">
    ...
    <drag :transfer-data="1">Inserisci un blocco del 8,33%</drag>
    <drag :transfer-data="2">Inserisci un blocco del 16,66%</drag>
    <drag :transfer-data="3">Inserisci un blocco del 25%</drag>
    <drag :transfer-data="4">Inserisci un blocco del 33,33%</drag>
    <drag :transfer-data="5">Inserisci un blocco del 41,66%</drag>
    <drag :transfer-data="6">Inserisci un blocco del 50%</drag>
    <drag :transfer-data="7">Inserisci un blocco del 58,33%</drag>
    <drag :transfer-data="8">Inserisci un blocco del 66,66%</drag>
    <drag :transfer-data="9">Inserisci un blocco del 75%</drag>
    <drag :transfer-data="10">Inserisci un blocco del 83,33%</drag>
    <drag :transfer-data="11">Inserisci un blocco del 91,66%</drag>
    <drag :transfer-data="12">Inserisci un blocco del 100%</drag>

    <div v-if="sameOrder" class="sameOrder ok">ORDINE OK</div>
    <div v-else class="sameOrder ko">ORDINE KO</div>

    <button v-if="sameOrder" type="button" @click.prevent="serialize">
      SERIALIZZA
    </button>
        ...
        <drop @drop="handleDrop">
          ...
          >
            <grid-item
              ...
              @moved="moved"
            >
              <button type="button" @click="removeMe(item.parentReference)">
                Delete
              </button>
              <button
                v-if="item.w !== 12"
...
export default {
  ...
  methods: {
    setBackgrounds() {
      this.$nextTick(() => {
        Array.from(this.$refs.ld.$el.querySelectorAll('.GridItem')).forEach(
          (el, idx) => (el.style.backgroundColor = '#' + seq[idx])
        );
        Array.from(this.$refs.ls.$el.querySelectorAll('.GridItem')).forEach(
          (el, idx) => (el.style.backgroundColor = '#' + seq[idx])
        );
        Array.from(this.$refs.lt.$el.querySelectorAll('.GridItem')).forEach(
          (el, idx) => (el.style.backgroundColor = '#' + seq[idx])
        );
      });
    ...
    serialize() {
      ...
      let allItems = this.sortedCopyLayout(this.layoutD).map(
        (i) => i.parentReference
      );
      ...
      if (document.getElementById('id_form_data')) {
        document.getElementById('id_form_data').value = JSON.stringify(
          this.serializedJson
        );
      }
    ...
    doAllContainersHasSameOrder() {
      ...
      const d = this.sortedCopyLayout(this.layoutD)
        .map((e) => e.i)
        .join('');
      const t = this.sortedCopyLayout(this.layoutT)
        .map((e) => e.i)
        .join('');
      const s = this.sortedCopyLayout(this.layoutS)
        .map((e) => e.i)
        .join('');
      return d === t && t === s;
    },
